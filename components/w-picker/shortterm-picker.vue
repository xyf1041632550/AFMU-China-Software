		...
		methods:{
			...
			getData(dVal){
				...
				for(let i=0;i<this.expand*1;i++){
					...
					let week=weeklist[new Date(year+"-"+this.formatNum(month)+"-"+this.formatNum(day)).getDay()];
					switch(i){
						case 0:
							label="今天";
							break;
					...
					dates.push({
						...
						// value:this.formatNum(month)+"-"+this.formatNum(day)
					})
				};
				for(let i=0;i<24;i++){
					hours.push({
				...
				for(let i=0;i<6;i++){
					minutes.push({
						label:this.formatNum(i*10),
						value:this.formatNum(i*10)
					})
				}
				return {
					dates,
