<template>
	<view class="range-slider" :style="'width:' + width + 'rpx;height:' + height + 'rpx'"  @click="onClickGroup">
...
export default {
	...
	methods: {
		...
		_onBlockTouchMove: function(e) {
			...
			if (tag == 'minBlock' || tag == 'maxBlock') {
				var that = this;
				var values = that._calculateValues(e,0);
				that._refreshProgressBar(values[2], values[3]);
				that._refreshBlock(values[2], values[3]);
				//拖动时也触发事件
				// var eventDetail = {
		...
		_onBlockTouchEnd: function(e) {
			...
			if (tag == 'minBlock' || tag == 'maxBlock') {
				var that = this;
				var values = that._calculateValues(e.mp.changedTouches[0],1);
				setTimeout(()=>{
					that._refreshProgressBar(values[2], values[3]);
					that._refreshBlock(values[2], values[3]);
				},40)
		 ...
		 * 根据手势计算相关数据
		 */
		_calculateValues: function(e,type=0) {
			//console.log(e)
			//console.log(type)
			var pageX = e.pageX;
			//兼容h5平台
			...
			//todo
			var MAX_LENGTH = this.width - this.blockSize;
			let step=MAX_LENGTH/this.max;
			//endtodo
			
			var that = this;
			var moveLength = pageX - that._blockDownX;
			var left = that._blockLeft + that._pxToRpx(moveLength);
			////console.log(moveLength,left)
			...
			let v1= Math.round(minLeft/step)*step;
			let v2=Math.round(maxLeft/step)*step;
			let vv1=type==1?v1:minLeft;
			let vv2=type==1?v2:maxLeft;
			
			// console.log(vv1,vv2)
			return [minValue, maxValue, vv1, vv2];
		},
		/**
		 * 计算滑块坐标
		 */
