	...
	methods: {
		...
		initCropper() {
			...
			if (containerRate > cropperRate) {
				...
				cropperRect.width =dbWidth|| this.containerRect.width * 0.85;
				cropperRect.height =dbWidth|| cropperRect.width / cropperRate;
			}
			if (cropperRate > imageRate) {
				imageRect.width = this.containerRect.width ;
				imageRect.height = this.containerRect.width  / imageRate;
			} else {
				cropperRect.width =dbWidth|| this.containerRect.width * 0.85;
				cropperRect.height =dbWidth|| cropperRect.width / cropperRate;
			...
			imageRect.top = (this.containerRect.height - imageRect.height) / 2;
			 cropperRect.left =(this.containerRect.width-dbWidth)/2
...
function updateImageStyle() {
	...
	console.log(changes.imageRect)
	console.log()
	if(isNaN (imageRect.left)||imageRect.left==null){
		imageRect.left=0;
	}
...
module.exports = {
	touchStart: function (ev, oi) {
		...
		if (startTouchs.length == 2) {
		 ...
		 imageRect.left=changes.imageRect.left;
		 imageRect.top=changes.imageRect.top;
		return false;
	},
	touchMove: function (ev, io) {
		...
		if (startTouchs.length == 1) {
			if (touchType === 'image') {
				console.log('changes.imageRect.left',imageRect.left,changes.imageRect.left,changeX1)
	...
	touchEnd: function (ev, oi) {
		...
		if (touchType === "image") {
			...
			console.log(cropperRect)
			if (changes.imageRect.left >cropperRect.left ) {
				changes.imageRect.left = cropperRect.left
			}
			if (changes.imageRect.left+changes.imageRect.width < cropperRect.left+cropperRect.width ) {
				changes.imageRect.left =cropperRect.left+cropperRect.width-changes.imageRect.width
			}
			if (changes.imageRect.top > cropperRect.top) {
				changes.imageRect.top = cropperRect.top
			}
			if (changes.imageRect.top+changes.imageRect.height < cropperRect.top+cropperRect.height) {
				changes.imageRect.top = cropperRect.top+cropperRect.height-changes.imageRect.height
			}
			// 边界控制end
			updateImageStyle()
		}
